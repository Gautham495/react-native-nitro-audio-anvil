...
/**
 * Wraps a Swift `(_ status: PermissionStatus) -> Void` as a class.
...
public final class Func_void_PermissionStatus {
  ...
  private let closure: (_ status: PermissionStatus) -> Void

  public init(_ closure: @escaping (_ status: PermissionStatus) -> Void) {
    self.closure = closure
  }

  @inline(__always)
  public func call(status: Int32) -> Void {
    self.closure(margelo.nitro.audioanvil.PermissionStatus(rawValue: status)!)
  }
