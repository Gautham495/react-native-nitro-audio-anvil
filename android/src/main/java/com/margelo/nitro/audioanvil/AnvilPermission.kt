...
internal object AnvilPermission {
  ...
  fun status(context: Context, activity: Activity?): PermissionStatus {
  ...
  fun request(context: Context, activity: Activity?): Promise<PermissionStatus> {
    val current = status(context, activity)
    if (current == PermissionStatus.GRANTED) return Promise.resolved(current)
    ...
    val listener = PermissionListener { _, _, grantResults ->
      ...
      promise.resolve(if (granted) PermissionStatus.GRANTED else PermissionStatus.DENIED)
      true
    }
